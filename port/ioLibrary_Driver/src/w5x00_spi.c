 ...
 * ----------------------------------------------------------------------------------------------------
 */
static inline void wizchip_select()
{
    gpio_put(WIZ_PIN_CS, 0);
}

static inline void wizchip_deselect()
{
...
void wizchip_reset()
{
    ...
    gpio_put(WIZ_PIN_RST, 0);
    sleep_ms(100);

    gpio_put(WIZ_PIN_RST, 1);
    sleep_ms(100);

    bi_decl(bi_1pin_with_name(WIZ_PIN_RST, "W5x00 RESET"));
}
...
static uint8_t wizchip_read()
{
    ...
    spi_read_blocking(WIZ_SPI_PORT, tx_data, &rx_data, 1);
...
{
    spi_write_blocking(WIZ_SPI_PORT, &tx_data, 1);
}
...
static void wizchip_critical_section_lock()
{
    critical_section_enter_blocking(&g_wizchip_cri_sec);
    //vPortEnterCritical();
}

static void wizchip_critical_section_unlock()
{
...
void wizchip_spi_initialize(uint32_t spi_clock)
{
    ...
    spi_init(WIZ_SPI_PORT, spi_clock);

    gpio_set_function(WIZ_PIN_SCK, GPIO_FUNC_SPI);
    gpio_set_function(WIZ_PIN_MOSI, GPIO_FUNC_SPI);
    gpio_set_function(WIZ_PIN_MISO, GPIO_FUNC_SPI);

    // make the SPI pins available to picotool
    bi_decl(bi_3pins_with_func(WIZ_PIN_MISO, WIZ_PIN_MOSI, WIZ_PIN_SCK, GPIO_FUNC_SPI));
    ...
    // make the SPI pins available to picotool
    bi_decl(bi_1pin_with_name(WIZ_PIN_CS, "W5x00 CHIP SELECT"));
